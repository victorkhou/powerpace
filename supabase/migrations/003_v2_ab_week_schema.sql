-- v2 Migration: AB-week PPL + Run schema
-- This is a breaking change — drops all existing program data and recreates tables.
-- Auth tables (profiles) are preserved.
...
-- programs (v2: AB-week, no fixed length)
...
-- workout_days (v2: week_type + variant for AB routing)
...
-- exercises (v2: bodyweight progression type, is_primary instead of is_primary_lift)
...
-- working_weights (unchanged structure)
...
-- sessions (v2: week_number, week_type, friday_alt instead of single 'week' column)
